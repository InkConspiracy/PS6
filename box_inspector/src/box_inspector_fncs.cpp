...
bool BoxInspector::model_poses_wrt_box(osrf_gear::Shipment &shipment_status) {
    ...
    for (int imodel = 0; imodel < num_models; imodel++) {
        if (imodel != i_box) { //if here, have a model NOT the box
            ...
            //MISSING LINES HERE...
            affine_part_wrt_box = xformUtils_.transformPoseToEigenAffine3d(part_pose_wrt_world)*affine_box_pose_wrt_world.inverse();
...
void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
     ...
     osrf_gear::Model product;
     osrf_gear::Product model_in_shipment;
     box_wrt_world = xformUtils_.transformPoseToEigenAffine3d(box_pose_wrt_world.pose);
     int num_models_in_shipment = shipment_wrt_box.products.size();
     for(int i = 0; i < num_models_in_shipment; i++){
        model_in_shipment = shipment_wrt_box.products[i];
        
        part_wrt_box = xformUtils_.transformPoseToEigenAffine3d(model_in_shipment.pose);
        part_wrt_world = part_wrt_box *box_wrt_world;
        geometry_msgs::Pose pose_part_wrt_world = xformUtils_.transformEigenAffine3dToPose(part_wrt_world);
        product.type = model_in_shipment.type;
        product.pose = model_in_shipment.pose;

        desired_models_wrt_world.push_back(product);
     }
